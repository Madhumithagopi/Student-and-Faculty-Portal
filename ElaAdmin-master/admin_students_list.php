...
if(isset($_GET['rs_id']))
	...
	if($opr=="del")
{
	$del_sql=mysql_query("DELETE FROM student_details WHERE stu_id=$id");
	if($del_sql)
...
<td><input type="text" class="text" name="searchtxt"  autocomplete="off" placeholder="search"/>
        ...
        <td>
        	...
        	<a href="admin_students_entry.php"><input type="button" name="butAdd" value="Add New" class="btn" /></a>
    </tr>
</table>
</form>
</div>
...
<form method="post">
    <table border="2" width="1050px" align="center" cellpadding="3"  cellspacing="0">
        <tr>
			...
			<th>Faculty Id-3</th>
			<th>Faculty Id-4</th>
			<th>Organisation Id-1</th>
			<th>Organisation Id-2</th>
			<th>Organisation Id-3</th>
			<th>Organisation Id-4</th>
	...
	if(isset($_POST['searchtxt']))
		$key=$_POST['searchtxt'];	
	if($key !="")
		$sql_sel=mysql_query("SELECT * FROM student_details WHERE name like '%$key%' OR regno like '%$key%' OR email like '%$key%' OR phone_no like '%$key%' OR batch like '%$key%' OR facid1 like '%$key%' OR orgid1 like '%$key%' OR semester like '%$key%'");
	else
		 $sql_sel=mysql_query("SELECT * FROM student_details");	
    $i=0;
	$e=1;
    ...
    ?>
      <tr bgcolor="<?php echo $color?>">
            ...
            <td><?php echo $row['facid1'];?></td>
			<td><?php echo $row['facid2'];?></td>
			<td><?php echo $row['facid3'];?></td>
			<td><?php echo $row['facid4'];?></td>
			<td><?php echo $row['orgid1'];?></td>
			<td><?php echo $row['orgid2'];?></td>
			<td><?php echo $row['orgid3'];?></td>
			<td><?php echo $row['orgid4'];?></td>
            <td><a href="everyone.php?tag=admin_student_update&opr=upd&rs_id=<?php echo $row['stu_id'];?>" title="Update"><img src="picture/update.png"/></a></td>
            <td><a href="?tag=admin_student_list&opr=del&rs_id=<?php echo $row['stu_id'];?>" title="Delete"><img src="picture/delete.png" /></a></td>
             </tr>
    <?php	
    }	
    ?>
