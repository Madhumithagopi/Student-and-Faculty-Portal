...
if(isset($_GET['rs_id']))
	...
	if($opr=="del")
{
	$del_sql=mysql_query("DELETE FROM guest_lectures WHERE g_id=$id");
	if($del_sql)
...
if($opr=="upd")
{
	$sql_upd=mysql_query("SELECT * FROM guest_lectures WHERE g_id=$id");
	...
	?>
        ...
        <div class="content">
            <div class="animated fadeIn">
                   ...
                   <div class="card">
                            <div class="card-body">
                                <h4 class="box-title">GUEST LECTURES LIST </h4>
                            </div>
							<div id="style_div" >
<form method="post">
<table width="755">
...
<form method="post">
    <table border="2" width="1050px" align="center" cellpadding="3"  cellspacing="0">
        <tr>
            <th>S.No.</th>
            <th>Title of Guest Lectures</th>
            <th>Date</th>
			<th>Resourceperson Id</th>
	...
	if(isset($_POST['searchtxt']))
		$key=$_POST['searchtxt'];	
	if($key !="")
		$sql_sel=mysql_query("SElECT * FROM guest_lectures WHERE res_id like '%$key%' OR title_of_lecture like '%$key%' OR date like '%$key%'");
	else
		 $sql_sel=mysql_query("SELECT * FROM guest_lectures");   
   $i=0;
    ...
    ?>
	  ...
	  <td><?php echo $i;?></td>
            <td><?php echo $row['title_of_lecture'];?></td>
            <td><?php echo $row['date'];?></td>
            <td><?php echo $row['res_id'];?></td>            
            <td><a href="everyone.php?tag=admin_guest_update&opr=upd&rs_id=<?php echo $row['res_id'];?>" title="Update"><img src="picture/update.png"/></a></td>
            <td><a href="?tag=admin_guest_list&opr=del&rs_id=<?php echo $row['g_id'];?>" title="Delete"><img src="picture/delete.png" /></a></td>
             </tr>
    <?php	
    }	
    ?>
