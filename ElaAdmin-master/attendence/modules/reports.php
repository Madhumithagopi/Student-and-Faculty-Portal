		...
		<div class="col-md-12">
			...
			<?php
				...
				if(($sdate<$t) && ($edate<=$t) && ($edate >= $sdate))
				{
					...
					$query_student = "SELECT student.sid, student.name, student.rollno from student ORDER BY student.sid";
						...
						for($j=$sdate;$j<=$edate;$j=$j+86400)
						{
							...
							$normalized_weekday = strtolower($weekday);
							 if(($normalized_weekday!="saturday") && ($normalized_weekday!="sunday"))
							 {
								 ...
								 $sql = "SELECT sid ,ispresent FROM attendance WHERE sid=$dsid AND date=$j AND $suid=uid " ;
								...
								{
									echo "<td><a href='index.php?&date=" . $currentDate . "'>TakeAttendance</a></td>";
								}
							}
						}
						if($totlec!=0)
