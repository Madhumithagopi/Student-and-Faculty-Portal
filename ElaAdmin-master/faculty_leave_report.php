...
$sql_str= "Select * from `student_leave_request` where `id` ='".$_REQUEST['c1']."' ";
        $res_str = mysql_query($sql_str);
        while ($row=mysql_fetch_array($res_str)){}
		
		  $sql = "update `student_leave_request`  set `status`='Approved' where `id` ='".$_REQUEST['c1']."'";	 								 
								$res = mysql_query($sql) or die ("could not insert!!! Kindly Check and submit");
$sql_str= "Select * from `student_leave_request` where `id` ='".$_REQUEST['c2']."' ";
        $res_str = mysql_query($sql_str);
        while ($row=mysql_fetch_array($res_str)){}
		
		  $sql = "update `student_leave_request` set `status`='Disapproved' where `id` ='".$_REQUEST['c2']."'";	 
								 $res = mysql_query($sql) or die ("could not insert!!! Kindly Check and submit");
    ...
    <div class="table-wrapper">
        <div class="table-title">
            <div class="row">
                ...
                <div class="col-sm-6">
				            <input type="submit" class="all" name="pending" value="Pending List">
        ...
        <table class="table table-striped table-hover">
            ...
            if($_POST["approve"])
			{
			$sql1="SELECT * FROM `student_leave_request` where `faculty_id`='".$_SESSION['empid']."' and `status`='Approved'";
			$res1= mysql_query($sql1);
            while($row1=mysql_fetch_array($res1)){
            echo"<td>"; echo $i++;echo "</td>";
            echo "<td>";echo $row1['name'];echo"</td>";
            echo"<td>"; echo $row1['regno'];echo"</td>";
			echo"<td>"; echo $row1['org_permission'];echo"</td>";
			echo"<td>"; echo $row1['leave_type'];echo"</td>";
			echo"<td>"; echo $row1['reason'];echo"</td>";
			
			echo "<td>";
			echo "<a href='faculty_leave_report.php?c1=$row1[id]'>";
			if($row1['status']!='Approved')
{
	echo"Approve\n<br>";
	}
	echo "</a>";
				if($row1['status']=='Approved'){echo"Approved\n|";}
 echo "<a href='faculty_leave_report.php?c2=$row1[id]'>";
 if($row1['status']!='DisApproved')
{
	echo"DisApprove\n<br>";
	}
	echo "</a>";
if($row1['status']=='Disapproved'){echo"DisApproved";}
echo"</td>";	
			echo"</tr>";
			}
			}
			...
			{
			$sql1="SELECT * FROM `student_leave_request` where `faculty_id`='".$_SESSION['empid']."' and `status`='Disapproved'";
			$res1= mysql_query($sql1);
            while($row1=mysql_fetch_array($res1)){
            echo"<td>"; echo $i++;echo "</td>";
            echo "<td>";echo $row1['name'];echo"</td>";
...
echo"</td>";	
			echo"</tr>";
				...
				{
			$sql1="SELECT * FROM `student_leave_request` where `faculty_id`='".$_SESSION['empid']."' and `status`='waiting for approval'";
			$res1= mysql_query($sql1);
            while($row1=mysql_fetch_array($res1)){
            echo"<td>"; echo $i++;echo "</td>";
            echo "<td>";echo $row1['name'];echo"</td>";
